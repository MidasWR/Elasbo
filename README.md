...
Репозиторий: [github.com/midaswr/elsabo](https://github.com/midaswr/elsabo)
...
```bash
curl -fsSL https://raw.githubusercontent.com/midaswr/elsabo/main/scripts/install.sh | sudo PREFIX=/usr/local/bin bash
```

Если релиза ещё нет или скачивание не удалось, скрипт попробует **`go install github.com/midaswr/elsabo/cmd/elsabo@latest`** (нужен [Go](https://go.dev/dl/) 1.25+).
...
```bash
git clone https://github.com/midaswr/elsabo.git
cd elsabo
make build
./dist/elsabo
```
